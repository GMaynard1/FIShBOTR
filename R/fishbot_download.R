...
#'
#' @param EPU the EPU of the dataset to download. Acceptable values include
#'  'GOM' for Gulf of Maine, 'GB' for Georges Bank, or 'MAB' for Mid-Atlantic
...
fishbot_download=function(EPU=NA,startDate,endDate){
  ## Select bounding box, if necessary
  if(is.na(EPU)==FALSE){
    coords=ifelse(
      EPU=='GOM',
      c(41.70484,44.58965,-71.09674,-67.66667),
      ifelse(
        EPU=='GB',
        c(39.83,42.33,-70,-65.66),
        ifelse(
          EPU=='MAB',
          c(36,41.87264,-77.22503,-70),
          stop("\n EPU Not Recognized, please select 'GOM','SNE', or 'MAB' or leave blank \n")
        )
      )
    )
  } else {
